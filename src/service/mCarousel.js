...
var Carousel = function () {
  this.init = function (el, opts) {
    ...
    this.centerX = this.wrapper.offsetWidth / 2 - 296;
    ...
    this.leftX = this._hasTransform2d ? this.wrapper.offsetWidth / 2 - 296 -545 : 0;
    // this.rightX = this._hasTransform2d ? this.viewW - this.singleW + this.diffX : this.viewW - this.singleW;
    this.rightX = this.wrapper.offsetWidth / 2 + 306 - 55;
    ...
    this._init(this.wrapper,this.length);
  },
  this._init = function(wrapper,length) {
    this._setup(wrapper);
  },
  this._bindEvent = function() {
    utils.on(this.wrapper, 'mousedown', this._start.bind(this));
    utils.on(this.wrapper, 'mousemove', this._move.bind(this));
    utils.on(this.wrapper, 'mouseup', this._end.bind(this));
  },
  ...
  this._setup = function(wrapper) {
    if (utils.css(wrapper, 'position') === 'static') wrapper.style.position = 'relative';
  ...
  this._setTransition = function(index, time) {
    time = time || 0;
    this.elements[index].style[this._prefixTransition] = 'all ' + time + 'ms';
  },
  this._setTransform = function(index, x, y, ratio) {
  ...
  this._setTransitionEnd = function() {
    this.animated = false;
  ...
  this._setTransitionStart = function(duration) {
  ...
  this._setStack = function(index, value) {
    this.elements[index].style.zIndex = value;
  },
  this._setStacks = function(dir) {
  ...
  this._setFourth = function(index, dir) {
    var dx = 0;
    if (dir === 'right') {
      this.dIndex = index - 1;
      dx = this.leftPos;
...
//   // this.singleH = opts.height || this.elements[this.index].offsetHeight;
//   // this.centerX = (this.viewW - this.singleW) / 2;
//   this.centerX = this.wrapper.offsetWidth / 2 - 306;
//   // this.centerY = (this.viewH - this.singleH) / 2;
//   console.log(this.singleW)
//   this.diffX = (this.singleW - this.singleW * this.ratio) / this.diff; //计算(transformOrigin = 'center center')的偏移
//   console.log(612 - this.diffX)
//   // this.leftX = this._hasTransform2d ? -this.diffX : 0;
//   this.leftX = this._hasTransform2d ? -71.5 * 2 : 0;
//   // this.rightX = this._hasTransform2d ? this.viewW - this.singleW + this.diffX : this.viewW - this.singleW;
//   this.rightX = this.wrapper.offsetWidth / 2 + 306 - 70;
//   this.spaceX = this.centerX - this.leftX;
//   this.leftPos = this.leftX - this.singleW * this.ratio;
//   this.rightPos = this.rightX + this.singleW * this.ratio;
//   this.rate = Math.abs((this.singleW * this.ratio) / this.spaceX);
